...
weston --backend=headless-backend.so --socket="$SOCKET" --width=1600 --height=900 &>"$TMP/weston.log" &
WPID=$!
sleep 2
DBUS_SESSION_BUS_ADDRESS="unix:path=/nonexistent" WAYLAND_DISPLAY="$SOCKET" \
    QT_FORCE_STDERR_LOGGING=1 \
    OSK_LAYOUT_SHOT="${OSK_LAYOUT_SHOT:-}" OSK_LAYOUT="${OSK_LAYOUT:-}" \
    timeout 60 qs -p "$(dirname "$0")/../OskLayoutProbe.qml" 2>&1 | grep -E "OskLayout|error|Error"
