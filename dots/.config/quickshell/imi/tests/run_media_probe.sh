...
WPID=$!
sleep 2
DBUS_SESSION_BUS_ADDRESS="unix:path=/nonexistent" WAYLAND_DISPLAY="$SOCKET" MEDIA_PROBE_SHOTS="${MEDIA_PROBE_SHOTS:-}" timeout 60 qs -p "$(dirname "$0")/../MediaTreeMotionProbe.qml" 2>&1 | grep "MediaTreeMotion"
